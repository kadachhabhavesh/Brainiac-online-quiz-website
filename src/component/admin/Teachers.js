...
export default () => {
    ...
    const loadUser = () => {
        fetch(`http://localhost:4000/user/admin/teachers`, { credentials: "include" })
            .then(res => {
                if (res.status === 401)
                    navigate("/")
                return res.json();
            }).then(res => { setTeachers(res); console.log(res); })
    ...
    const filteredStudents = teachers && teachers.filter((teachers) => `${teachers.firstname} ${teachers.lastname}`.toLowerCase().includes(searchText.toLowerCase()) || teachers.email.toLowerCase().includes(searchText.toLowerCase()))

    return <div className={`flex gap-y-4 flex-col mt-4`}>
        <div className="w-full max-h-[96vh] rounded flex flex-col gap-x-2 bg-white shadow-lg px-2 overflow-x-scroll no-scrollbar">
            ...
            {filteredStudents && filteredStudents.length <= 0 && <h1 className="p-2 mx-auto font-bold text-gray-500 mb-2">Teachers Not Found</h1>}
            {filteredStudents && filteredStudents.length > 0 && <table class="w-full h-fit text-sm text-left rtl:text-right text-gray-500 rounded">
                ...
                <tbody>
                    ...
                    {filteredStudents.map((tea, index) => <tr class="bg-white border-b hover:bg-gray-50" >
                        ...
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            {tea.Emailverified ? <label className="bg-sky-400 px-2 py-0.5 rounded-full text-xs text-white">verified</label> : <label className="bg-red-400 px-2 py-0.5 rounded-full text-xs text-white">not verified</label>}
                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            {tea.password}
                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            {tea.totalquizcreated}
                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            {tea.totalplays}
                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap flex items-center">
                            ...
                            <svg className="h-6" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" onClick={() => navigate(`/admin/userprofile/${tea._id}`)}>
                            ...
                            <svg onClick={() => deleteUser(tea._id)} className="h-7 hover:bg-gray-200 rounded p-1 stroke-red-500 fill-red-500" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 30 30">
                                <path d="M 14.984375 2.4863281 A 1.0001 1.0001 0 0 0 14 3.5 L 14 4 L 8.5 4 A 1.0001 1.0001 0 0 0 7.4863281 5 L 6 5 A 1.0001 1.0001 0 1 0 6 7 L 24 7 A 1.0001 1.0001 0 1 0 24 5 L 22.513672 5 A 1.0001 1.0001 0 0 0 21.5 4 L 16 4 L 16 3.5 A 1.0001 1.0001 0 0 0 14.984375 2.4863281 z M 6 9 L 7.7929688 24.234375 C 7.9109687 25.241375 8.7633438 26 9.7773438 26 L 20.222656 26 C 21.236656 26 22.088031 25.241375 22.207031 24.234375 L 24 9 L 6 9 z"></path>
                            </svg>
                            <svg onClick={() => openEditModel(tea)} className="h-7 hover:bg-gray-200 rounded p-1 stroke-teal-500 fill-teal-500" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 24 24">
