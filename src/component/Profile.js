...
export default () => {
    ...
    return student && <div className="w-full p-4 flex flex-col gap-y-2" >
        ...
        {/* tab views */}
        {<div className="w-full rounded min-h-[680px]">
            {student.results.length <= 0 && tab === 0 && <h1 className="text-center p-4 font-bold text-gray-600">No Results Found</h1>}
            {student.results.length > 0 && tab === 0 && <div className="flex h-[calc(100vh-100px)] gap-x-2">
                {/* result list */}
                <div className="w-4/12 h-full rounded flex flex-col gap-y-2" >
                    <h1 className="bg-white h-[7%] font-semibold rounded flex items-center px-4">{student.results.length} Results</h1>
