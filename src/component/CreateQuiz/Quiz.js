      ...
      <div className="p-5">
        ...
        <div className="flex gap-x-4 mt-5">
          <div className="ml-0 max-w-fit min-w-fit">
            ...
            {quizdetails.image ? (
              <div className="w-72 h-[135px] border border-dashed rounded relative">
                <img
                  className="w-full h-full rounded"
                  src={ (typeof(delquizid) == "undefined" || typeof(quizdetails.image) === 'object') ? URL.createObjectURL(quizdetails.image) :`http://localhost:4000/${quizdetails.image}`}
                />
                <button
                  className="absolute top-0 right-0 m-2 bg-red-500 text-white py-1 px-2 rounded hover:bg-red-700"
                  onClick={() => setQuizdetails({ ...quizdetails, image: null })}
                >
                  Delete
                </button>
              </div>
