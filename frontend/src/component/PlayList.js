...
export default () => {
    ...
    return <>
        ...
        <div className='p-2'>
            { user && user.usertype==="teacher" && <div className='w-fit px-3 py-1 my-2 bg-white rounded font-bold flex gap-x-2 items-center cursor-pointer' onClick={() => {
            ...
            {playList && playList.length > 0 && <div className='w-full flex flex-wrap gap-3'>
                ...
                <div className='w-[calc(60%-10px)] h-[90vh] bg-white rounded border px-3 overflow-y-scroll no-scrollbar flex flex-col gap-y-2'>
                    ...
                    {playList && playList.length > 0 && playList[currentPlayList] && playList[currentPlayList].quizziz.map(quiz => <div className="w-full h-24 bg-white rounded flex gap-x-2 p-2 hover:bg-gray-100 hover:border border-2 " onClick={() => navigate(`/home/playquiz/${quiz._id}`)}>
